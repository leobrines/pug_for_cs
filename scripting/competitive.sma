...
new money_sprites[MAX_PLAYERS+1][NUMBER_OF_SPRITES];
new tmpstr[6], value[2];
new dolar_sprite[MAX_PLAYERS+1];
...
new weapon_sprites[33][NUMBER_OF_WEAPTYPES];
new ArmorType:armor, weapons;
...
public display_user_sprites (id) {
	display_money(id);
	display_weapons(id);
}
...
public display_money_digit (id, sprite, Float:frame, amount) {
	switch (sprite) {
		case 0:
...
public display_c4_or_defuse (id) {
	if (weapons & 1<<CSW_C4)
...
public drawspr (const ent, player_id, str[], Float:frame, Float:offset, amount, r, g, b) {
	if (!is_entity(ent))
		return;
